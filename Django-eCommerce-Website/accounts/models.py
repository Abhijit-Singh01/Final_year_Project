...
class CartItem(BaseModel):
    ...
    def get_product_price(self):
        """
        Returns line total (unit price + variant price) * quantity.
        """
        price = 0
        ...
        if self.color_variant and self.color_variant.price:
            price += int(self.color_variant.price)

        return price * int(self.quantity or 0)
...
class OrderItem(BaseModel):
    ...
    def get_total_price(self):
        """
        Returns the line total for this order item.

        Note: size variants were removed from this project, so we must not
        reference `size_variant` here.
        """
        if self.product_price is None:
            return Decimal("0.00")
        return Decimal(self.product_price)
